...
int main(){
	...
	string filename = "StaffManagementSystem.dat";
...
void program(vector<Staff>& records, int choice) {
    ...
    switch(choice){
        case 1:
			...
			while ( indexFound != -1 ) {
				int id;
				cout << "Please reenter the ID :" << endl;
				cout << "ID   : "; cin >> id;
				theObject.setID(id);
			...
			ofstream outFile;
			outFile.open("StaffManagementSystem.dat", ios::app);
...
void readFromFile(vector<Staff>& records, const string& filename) {
	...
	while (!inFile.eof()) {
		...
		if (line.length() > 0) {
			...
			int buildingNumber = atoi(data[3].c_str());
			int zipCode = atoi(data[7].c_str());
			Address addr(buildingNumber, data[4], data[5], data[6], zipCode);
			role = data[8];
	...
	cout << "Number of records loaded from the master file : " << records.size() << endl;
	inFile.close();
}
